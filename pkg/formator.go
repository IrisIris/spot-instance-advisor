...
func (md *MarkDownFormator) getMaxLength() error {
	...
	jsonStr, err := json.Marshal(*(md.MaxlengthData))
	if err != nil {
		return err
	}
...
func getFormator(formatType string, data *[]AdvisorResponse, haveHeader bool, maxlengthData *[]AdvisorResponse) (Formator, error) {
	switch formatType {
	case "markdown":
...
func DoFormat(formatType string, toFormat *[]AdvisorResponse, haveHeader bool, maxlengthData *[]AdvisorResponse) (string, error) {
	if len(*toFormat) == 0 {
		return "", fmt.Errorf("nothing to format")
	}
	formator, err := getFormator(formatType, toFormat, haveHeader, maxlengthData)
